...
const FIELDS = [
  { key: 'hero_title',        label: 'Hero titel',          hint: 'Gebruik \\n voor een nieuwe regel' },
  { key: 'hero_subtitle',     label: 'Hero ondertitel',     hint: '' },
  { key: 'hero_cta_balloon',  label: 'CTA ballon tekst',    hint: 'Bijv. "Ik wil meelopen →"' },
  { key: 'urgency_starburst', label: 'Starburst tekst',     hint: 'Oranje badge rechtsboven CTA. Leeg = verbergen.' },
  { key: 'urgency_sticky',    label: 'Sticky label',        hint: 'Geel briefje onder CTA. Leeg = verbergen.' },
  { key: 'home_intro_text',   label: 'Intro tekst (onder video)', hint: 'Blauwe sectie onder het filmpje. Leeg = alleen golflijntjes.' },
]
...
export default function LandingspaginaAdminPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f7faf9', fontFamily: 'Kodchasan, sans-serif', padding: '1.5rem 1rem' }}>
      <style>{`.lp-input:focus { border-color: #3b696d !important; box-shadow: 0 0 0 2px rgba(59,105,109,0.15); }`}</style>
      <div style={{ maxWidth: '48rem', margin: '0 auto' }}>
        <Link href="/admin/dashboard" style={{ fontSize: '13px', color: '#3b696d', fontWeight: 600, textDecoration: 'none' }}>← Dashboard</Link>
        <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: '#1f2937', margin: '0.25rem 0 0' }}>Landingspagina</h1>
        <p style={{ color: '#6b7280', fontSize: '14px', margin: '0.25rem 0 1.5rem' }}>Hero sectie en intro tekst</p>
