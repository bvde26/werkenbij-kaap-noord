...
export default function UspsAdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    for (const key of Object.keys(map)) {
      const m = key.match(/^home_usp_(\d+)_(titel|tekst)$/)
  ...
  const save = async (num: number, field: 'titel' | 'tekst') => {
    ...
    if (error) setError(error.message)
      ...
      setUsps(prev => prev.map(v => v.num === num ? { ...v, [`${field}Dirty`]: false } : v))
      setSaved(id)
  ...
  const deleteUsp = async (num: number) => {
  ...
  const addUsp = async () => {
    setAdding(true)
    const maxNum = usps.length > 0 ? Math.max(...usps.map(v => v.num)) : 0
    const n = maxNum + 1
    await supabase.from('site_content').insert([
      { key: `${PREFIX}${n}_titel`, value: '', label: `Waarom KN ${n} — titel`, page: PAGE },
      { key: `${PREFIX}${n}_tekst`, value: '', label: `Waarom KN ${n} — tekst`, page: PAGE },
    ])
    await load()
    setAdding(false)
  }
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f7faf9', fontFamily: 'Kodchasan, sans-serif', padding: '1.5rem 1rem' }}>
      <style>{`.usp-input:focus { border-color: #3b696d !important; box-shadow: 0 0 0 2px rgba(59,105,109,0.15); }`}</style>
      <div style={{ maxWidth: '48rem', margin: '0 auto' }}>
        ...
        <p style={{ color: '#6b7280', fontSize: '14px', margin: '0.25rem 0 1.5rem' }}>
          Homepage — &ldquo;Waarom Kaap Noord?&rdquo; sectie
        </p>
